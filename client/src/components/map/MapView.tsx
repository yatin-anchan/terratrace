...
function makeMaterialMarker(icon: string, color: string, size = 20, label?: string) {
  ...
  iconEl.style.textShadow = `0 0 10px ${color}66`
  ...
  switch (style) {
    case 'SAT':
      return {
        ...
        layers: [{ id: 'sat', type: 'raster', source: 'sat', paint: { 'raster-saturation': -0.2, 'raster-brightness-max': 0.7 } }],
      }
    case 'LIGHT':
      return {
        version: 8,
        ...
        layers: [{ id: 'osm', type: 'raster', source: 'osm', paint: { 'raster-brightness-min': 0.85, 'raster-brightness-max': 1, 'raster-saturation': -0.3 } }],
      }
    case 'TOPO':
      return {
        version: 8,
        ...
        layers: [{ id: 'topo', type: 'raster', source: 'topo', paint: { 'raster-brightness-max': 0.6, 'raster-saturation': -0.4 } }],
      }
    case 'CONTOUR':
      return {
        version: 8,
        ...
        layers: [{ id: 'osm', type: 'raster', source: 'osm', paint: { 'raster-brightness-max': 0.1, 'raster-saturation': -1, 'raster-contrast': 0.3 } }],
      }
    case '3D':
      return {
        version: 8,
        ...
        layers: [{ id: 'sat', type: 'raster', source: 'sat', paint: { 'raster-saturation': -0.2 } }],
      }
    case 'TRAFFIC':
      return {
        version: 8,
        ...
        layers: [{ id: 'osm', type: 'raster', source: 'osm', paint: { 'raster-saturation': -0.8, 'raster-brightness-max': 0.2 } }],
      }
    case 'WEATHER':
      return {
        version: 8,
        ...
        layers: [{ id: 'osm', type: 'raster', source: 'osm', paint: { 'raster-saturation': -0.7, 'raster-brightness-max': 0.25 } }],
      }
    case 'POPULATION':
      return {
        version: 8,
        ...
        layers: [{ id: 'osm', type: 'raster', source: 'osm', paint: { 'raster-saturation': -0.6, 'raster-brightness-max': 0.3, 'raster-hue-rotate': 200 } }],
      }
    default:
      return {
        version: 8,
        ...
        layers: [{ id: 'osm', type: 'raster', source: 'osm', paint: { 'raster-brightness-max': 0.15, 'raster-saturation': -1, 'raster-contrast': 0.2 } }],
      }
  }
}
...
export default function MapView({
  ...
  const onPickLocationRef = useRef<typeof onPickLocation>(undefined)
  ...
  const loadEvidence = useCallback(async (map: maplibregl.Map) => {
    ...
    try {
      ...
      items.forEach((ev) => {
        ...
        const el = makeMaterialMarker('location_on', color, 20)
  ...
  const loadSubjects = useCallback(async (map: maplibregl.Map) => {
    ...
    try {
      ...
      subs.forEach((s) => {
        ...
        const el = makeMaterialMarker('person_pin_circle', '#cc4444', 26)
  ...
  const loadBasecamps = useCallback(async (map: maplibregl.Map) => {
    ...
    try {
      ...
      items.forEach((b) => {
        ...
        const el = makeMaterialMarker('camping', '#3a9e6a', 22, b.name ?? 'BASE')
    ...
    if (cfg.lineCoords) {
      ...
      if (!map.getSource(srcId)) {
        map.addSource(srcId, {
          ...
          data: {
            type: 'Feature',
            geometry: { type: 'LineString', coordinates: cfg.lineCoords },
            properties: {},
          },
        })
        ...
        map.addLayer({
          ...
          paint: {
            ...
            'line-width': layerId === 'roads' ? 2.5 : 1.5,
            'line-opacity': 0.6,
            'line-dasharray': layerId === 'rivers' ? [1, 2] : [1, 0],
          },
        })
      }
    }
  ...
  const plotSimulation = useCallback((map: maplibregl.Map, hs: typeof hotspots, paths: typeof samplePaths) => {
    ...
    map.addSource('hotspot-circles', {
      ...
      data: {
        ...
        features: hs.map((h) => ({
          ...
          properties: { probability: h.probability },
    ...
    hs.forEach((h, i) => {
      ...
      hotspotMarkersRef.current.push(
        new maplibregl.Marker({ element: el, anchor: 'bottom' }).setLngLat([h.lng, h.lat]).addTo(map)
      )
    })
    ...
    if (paths.length > 0) {
      ...
      map.addSource('agent-paths', {
        ...
        data: {
          type: 'FeatureCollection',
          features: paths.map((a) => ({
            type: 'Feature' as const,
            properties: { behavior: a.behavior },
            geometry: {
      ...
      map.addLayer({
        ...
        source: 'agent-paths',
        paint: {
          'line-color': ['match', ['get', 'behavior'], 'trail_follower', bc.trail_follower, 'downhill', bc.downhill, 'random_walk', bc.random_walk, 'shelter_seek', bc.shelter_seek, 'stationary', bc.stationary, '#e8c87a'],
          'line-width': 0.8,
          'line-opacity': 0.22,
        },
      })
  ...
  useEffect(() => {
    ...
    map.on('click', (e) => {
      if (!pickModeRef.current || !onPickLocationRef.current) return
      ...
      onPickLocationRef.current(picked)

      showPopup(
        map,
        [picked.lng, picked.lat],
        `<div class="tt-popup-inner">
          <div class="tt-popup-title" style="color:var(--accent)">LOCATION PICKED</div>
          <div class="tt-popup-row">Lat: <span>${picked.lat}</span></div>
          <div class="tt-popup-row">Lng: <span>${picked.lng}</span></div>
        </div>`
      )
    })
  ...
  }, [loadAllData, onZoomChange, showPopup])
  ...
  }, [mapStyle, mapReady, loadAllData, activeLabelLayers, addLabelLayerToMap, hasResults, hotspots, samplePaths, plotSimulation])
  ...
  return subscribeOperationRefresh(operationId, () => {
    loadAllData(map)
  })
  ...
  return (
    <div className={styles.wrap}>
      ...
      {pickMode && (
        ...
        >
          PICK MODE ACTIVE — CLICK ANY POINT ON THE MAP
        </div>
      )}
      ...
      <div className={styles.layerPanel}>
        ...
        {LAYER_TOGGLES.map((l) => (
          <button
            key={l.id}
            className={`${styles.layerToggle} ${visibleLayers.has(l.id) ? styles.layerToggleOn : ''}`}
            onClick={() => toggleLayer(l.id)}
          >
            <span
              className={styles.layerDot}
              style={{ background: visibleLayers.has(l.id) ? l.color : '#2a3048' }}
            />
            {l.label}
            {l.id === 'hotspot-circles' && hasResults && (
              <span style={{ marginLeft: 'auto', fontSize: 9, color: '#3a9e6a' }}>LIVE</span>
            )}
          </button>
        ))}
        ...
        {LABEL_LAYERS.map((l) => (
          <button
            key={l.id}
            className={`${styles.layerToggle} ${activeLabelLayers.has(l.id) ? styles.layerToggleOn : ''}`}
            onClick={() => toggleLabelLayer(l.id)}
          >
            <span
              className={styles.layerDot}
              style={{ background: activeLabelLayers.has(l.id) ? l.color : '#2a3048' }}
            />
            {l.label}
          </button>
        ))}
      ...
      <div className={styles.coordBar}>
        ...
        {mapStyle === 'TRAFFIC' && (
          <span className={styles.wxBadge} style={{ color: '#c0803a' }}>
            TRAFFIC LIVE
          </span>
        )}
        {mapStyle === '3D' && <span className={styles.wxBadge}>3D TERRAIN</span>}
        {mapStyle === 'POPULATION' && (
          <span className={styles.wxBadge} style={{ color: '#ff6040' }}>
            POPULATION DENSITY
          </span>
        )}
        {hasResults && (
          <span className={styles.wxBadge} style={{ color: '#3a9e6a' }}>
            SIM PLOTTED
          </span>
        )}
        {pickMode && (
          <span className={styles.wxBadge} style={{ color: 'var(--accent)' }}>
            PICK MODE
          </span>
        )}
      </div>
